...
# -*- coding: utf-8 -*-
"""
Generador simple de archivos para It's English Time
"""
...
def main():
    ...
    for professor_name, students in PROFESORES.items():
        print(f"📝 Generando panel de {professor_name.title()}...")
        
        html_content = create_professor_panel(professor_name, students)
        filename = f"panel_{professor_name}.html"
        
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(html_content)
        
        print(f"✅ Panel de {professor_name.title()} creado: {filename}")
    
    # Generar páginas de alumnos
    for professor_name, students in PROFESORES.items():
        for student_name in students:
            print(f"📝 Generando página de {student_name}...")
            ...
            print(f"✅ Página de {student_name} creada: {filename}")
    
    print("\n🎉 ¡Todos los archivos han sido generados exitosamente!")
    print(f"📊 Resumen:")
    print(f"   - {len(PROFESORES)} paneles de profesores")
    
    total_students = sum(len(students) for students in PROFESORES.values())
    print(f"   - {total_students} páginas de alumnos")
    print(f"   - Total: {len(PROFESORES) + total_students} archivos HTML")
